# ==========================================================
# TUGAS HANDS-ON MODUL 1
# Studi Kasus: Sistem Stok Barang Kantin (Berbasis File .txt)
#
# Nama  : Muhamad Fatir Kurniawan
# NIM   : J0403251057
# Kelas : A1
# ==========================================================
# -------------------------------
# Konstanta nama file
# -------------------------------
NAMA_FILE = "stok_barang.txt"

# -------------------------------
# Fungsi: Membaca data dari file
# -------------------------------
def baca_stok(nama_file):
    """
    Membaca data stok dari file teks.
    Format per baris: KodeBarang,NamaBarang,Stok
    Output:
    - stok_dict (dictionary)
      key = kode_barang
      value = {"nama": nama_barang, "stok": stok_int}
    """
    stok_dict = {}
    # Buka file dan baca seluruh baris
    with open(nama_file, "r", encoding="utf-8") as f:
        for baris in f:
            baris = baris.strip() # Hapus spasi dan newline
            kode_barang, nama_barang, stok = baris.split(',') # Pisah berdasarkan koma
            stok_dict[kode_barang] = {"nama": nama_barang, "stok": int(stok)} # Simpan ke dictionary

    return stok_dict # Kembalikan dictionary

# -------------------------------
# Fungsi: Menyimpan data ke file
# -------------------------------
def simpan_stok(nama_file, stok_dict):
    """
    Menyimpan seluruh data stok ke file teks.
    Format per baris: KodeBarang,NamaBarang,Stok
    """
    # Tulis ulang seluruh isi file berdasarkan stok_dict
    with open(nama_file, "w", encoding="utf-8") as f:
        for kode_barang, info in stok_dict.items():
            nama_barang = info["nama"] # ambil nama barang
            stok = info["stok"] # ambil stok barang
            # tulis ke file dengan format yang benar
            f.write(f"{kode_barang},{nama_barang},{stok}\n")

# -------------------------------
# Fungsi: Menampilkan semua data
# -------------------------------
def tampilkan_semua(stok_dict):
    """
    Menampilkan semua barang di stok_dict.
    """
    # Jika kosong, tampilkan pesan stok kosong
    # Tampilkan dengan format rapi: kode | nama | stok
    if not stok_dict:
        print("Stok barang kosong.")
        return
    
    print("\n=== DAFTAR STOK BARANG KANTIN ===")
    print(f"{'Kode Barang':<15} | {'Nama Barang':<30} | {'Stok':<5}")
    print("-" * 60)
    for kode in sorted(stok_dict.keys()):
        nama = stok_dict[kode]["nama"] # ambil nama barang
        stok = stok_dict[kode]["stok"] # ambil stok barang
        print(f"{kode:<15} | {nama:<30} | {stok:>5}") # tampilkan data
    print("-" * 60)

# -------------------------------
# Fungsi: Cari barang berdasarkan kode
# -------------------------------
def cari_barang(stok_dict):
    """
    Mencari barang berdasarkan kode barang.
    """
    kode = input("Masukkan kode barang: ").strip()
    # Cek apakah kode ada di dictionary
    if kode in stok_dict:
        # Ambil info barang
        info = stok_dict[kode]
        # Tampilkan detail barang
        print(f"\n\nDetail barang dengan kode {kode}:")
        print(f"Nama Barang: {info['nama']}")
        print(f"Stok: {info['stok']}")
    else:
        print("Barang tidak ditemukan.")

# -------------------------------
# Fungsi: Tambah barang baru
# -------------------------------
def tambah_barang(stok_dict):
    """
    Menambah barang baru ke stok_dict.
    """
    kode = input("Masukkan kode barang baru: ").strip()
    nama = input("Masukkan nama barang: ").strip()
    # Validasi kode tidak boleh duplikat
    if kode in stok_dict:
        print("Kode sudah digunakan.")
        return
    # Input stok awal (integer)
    try:
        stok_awal = int(input("Masukkan stok awal (angka): ").strip())
    except ValueError:
        print("Stok harus berupa angka.")
        return

    # Simpan ke dictionary
    stok_dict[kode] = {"nama": nama, "stok": stok_awal}

# -------------------------------
# Fungsi: Update stok barang
# -------------------------------
def update_stok(stok_dict):
    """
    Mengubah stok barang (tambah atau kurangi).
    Stok tidak boleh menjadi negatif.
    """
    kode = input("Masukkan kode barang yang ingin diupdate: ").strip()
    # Cek apakah kode ada di dictionary
    if kode not in stok_dict:
        print("Barang tidak ditemukan.")
        return
    stok = stok_dict[kode]["stok"]
    print("Pilih jenis update:")
    print("1. Tambah stok")
    print("2. Kurangi stok")

    try:
        pilihan = int(input("Masukkan pilihan (1/2): ").strip())
        jumlah = int(input("Masukkan jumlah: ").strip())
    except ValueError:
        print("Input harus berupa angka.")
        return


    # Input jumlah perubahan stok
    # - Jika pilihan 1: stok = stok + jumlah
    # - Jika pilihan 2: stok = stok - jumlah
    # - Jika hasil < 0: batalkan dan tampilkan error
    if pilihan == 1:
        stok += jumlah
        stok_dict[kode]["stok"] = stok
        print(f"Stok barang {kode} berhasil ditambah. Stok baru: {stok}")
    elif pilihan == 2:
        if stok - jumlah < 0:
            print("Stok tidak boleh negatif. Update dibatalkan.")
            return
        stok -= jumlah
        stok_dict[kode]["stok"] = stok
        print(f"Stok barang {kode} berhasil dikurangi. Stok baru: {stok}")
    else:
        print("Pilihan tidak valid.")
        return

# -------------------------------
# Program Utama
# -------------------------------
def main():
    # Membaca data dari file saat program mulai
    stok_barang = baca_stok(NAMA_FILE)
    while True:
        print("\n=== MENU STOK KANTIN ===")
        print("1. Tampilkan semua barang")
        print("2. Cari barang berdasarkan kode")
        print("3. Tambah barang baru")
        print("4. Update stok barang")
        print("5. Simpan ke file")
        print("0. Keluar")
        pilihan = input("Pilih menu: ").strip()
        if pilihan == "1":
            tampilkan_semua(stok_barang)
        elif pilihan == "2":
            cari_barang(stok_barang)
        elif pilihan == "3":
            tambah_barang(stok_barang)
        elif pilihan == "4":
            update_stok(stok_barang)
        elif pilihan == "5":
            simpan_stok(NAMA_FILE, stok_barang)
            print("Data berhasil disimpan.")
        elif pilihan == "0":
            print("Program selesai.")
            break
        else:
            print("Pilihan tidak valid. Silakan coba lagi.")

# Menjalankan program utama
if __name__ == "__main__":
    main()
